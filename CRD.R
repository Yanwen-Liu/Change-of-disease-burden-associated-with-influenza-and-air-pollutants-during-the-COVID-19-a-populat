...
ggsave("result/xgboost_varaible_attr_crd.pdf", ppl)




pdf('result/xgboost_imp_crd.pdf')
par(mfrow = c(3, 2))
for (i in df_list){
...
write.csv(annual_excess,'result/annual_excess_crd.csv',quote=F,row.names = FALSE)
...
ggsave("result/xgboost_sensitivity_crd.pdf", ppl)
sensitivity_plot[2019]

write.csv(eval_matrix,'result/model_valdiation_evaluation_crd.csv',quote=F,row.names = FALSE)
