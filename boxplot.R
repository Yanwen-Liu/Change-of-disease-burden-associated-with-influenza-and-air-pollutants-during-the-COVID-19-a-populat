...
for (i in df_list){
  ...
  temp<-df%>%filter(year>2014)%>%mutate(mortality_rate=1000*weekly_death/pop,age_group=df_name[which(df_list==i)])%>%
    select(year,mortality_rate,age_group)
  temp$year[temp$year<2020]<-'Pre Pandemic'
  temp$year[temp$year==2020]<-'Pandemic'
...
ggplot(temp, aes(x=age_group, y=mortality_rate, fill=Category)) +
  geom_boxplot()+
