...
ggsave("result/xgboost_varaible_attr.pdf", ppl)
...
ggplot(data=plot_df,aes(x=week_number))+geom_line(aes(y=actual_death,color='Observed (Covid death excluded)'))+
  ...
  scale_color_manual(name = "Method", 
                     labels = c('GAM', 'Observed (Covid death excluded)','XGBoost'),
...
for (i in age_list){
  ...
  plot_df%>%filter(week_number>991)%>%mutate(excess_gam=actual_death-predicted_gam,
                                           excess_xg=actual_death-predicted_xg)%>%
  group_by(age_group)%>%summarise(xg=sum(excess_xg),gam=sum(excess_gam))





write.csv(annual_excess,'result/annual_excess_all_cause.csv',quote=F,row.names = FALSE)

attr<-variable_attr%>%group_by(year,age_group,variable)%>%summarise(yearly_attri=sum(death_attr))
write.csv(attr,'result/variable_attr_all.csv',quote=F,row.names = FALSE)
...
for (j in test_year){
  ...
  scale_x_continuous(breaks = round(seq(from=min(df$idx),to=max(df$idx)+1,by=52)),
      ...
      labs(title=paste( 'Using',as.character(j-10), 'to',as.character(j-1),
                        'data to predict weekly all cause death counts \n of different age groups in',as.character(j)),
           x='year',y='death counts')+
...
ggsave("result/xgboost_sensitivity.pdf", ppl)
sensitivity_plot[2019]

write.csv(eval_matrix,'result/model_valdiation_evaluation.csv',quote=F,row.names = FALSE)
...
#####
#####
df_max<-subset(df_max,year>2009)
layout(matrix(c(1,2,3,4,5,6), 3, 2, byrow = TRUE))
plot(df_max$start_date, df_max$flu_a ,type='l',xlab = "date", ylab = "Flu A") 
plot(df_max$start_date, df_max$flu_b ,type='l',xlab = "date", ylab = "Flu b")
plot(df_max$start_date, df_max$no2 ,type='l',xlab = "date", ylab = "NO2")
plot(df_max$start_date, df_max$o3 ,type='l',xlab = "date", ylab = "O3")
plot(df_max$start_date, df_max$pm2.5 ,type='l',xlab = "date", ylab = "PM 2.5")
plot(df_max$start_date, df_max$pm10 ,type='l',xlab = "date", ylab = "PM 10")
...
data_test<-df[rep(500,20),]
data_test$pop<-seq(118,217,5)
xg<-xgb.DMatrix(as.matrix(data_test[,-c(2,3,6,15)]), 
                label = data_test$weekly_death)
setinfo(xg, "base_margin", log(data_test$pop))
# weight = df$pop

pred<-predict(final,xg)
plot(data_test$pop,pred)





data_test<-df[rep(500,11),]
data_test$year<-seq(2010,2020,1)
data_test$year<-year(data_test$year)
xg<-xgb.DMatrix(as.matrix(data_test[,-c(2,3,6,15)]), 
                label = data_test$weekly_death)
setinfo(xg, "base_margin", log(data_test$pop))
# weight = df$pop
