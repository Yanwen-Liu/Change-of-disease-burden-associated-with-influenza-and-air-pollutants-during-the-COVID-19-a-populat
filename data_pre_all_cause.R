...
library(lubridate)
library(readr)
weekly_all_cause_death <- read_csv("weekly_all_cause_death_with_age_groups.csv")
...
weekly_all_cause_death$idx<-seq.int(nrow(weekly_all_cause_death))
...
df_all<-weekly_all_cause_death%>%select(weekly_death,idx)%>%left_join(
  weather_and_air,by=c('idx'='idx'))%>%merge(all_pop,by=c('year'='year'))

df_20<-weekly_all_cause_death%>%select(weekly_death_20,idx)%>%left_join(
  weather_and_air,by=c('idx'='idx'))%>%merge(pop_20,by=c('year'='year'))

df_40<-weekly_all_cause_death%>%select(weekly_death_40,idx)%>%left_join(
  weather_and_air,by=c('idx'='idx'))%>%merge(pop_40,by=c('year'='year'))

df_65<-weekly_all_cause_death%>%select(weekly_death_65,idx)%>%left_join(
  weather_and_air,by=c('idx'='idx'))%>%merge(pop_65,by=c('year'='year'))

df_85<-weekly_all_cause_death%>%select(weekly_death_85,idx)%>%left_join(
  weather_and_air,by=c('idx'='idx'))%>%merge(pop_85,by=c('year'='year'))

df_max<-weekly_all_cause_death%>%select(weekly_death_max,idx)%>%left_join(
  weather_and_air,by=c('idx'='idx'))%>%merge(pop_max,by=c('year'='year'))

save(df_all,file = 'all_cause_data/all_age_weekly_mortality.rd')
save(df_20,file = 'all_cause_data/0-20_weekly_mortality.rd')
save(df_40,file = 'all_cause_data/20-40_weekly_mortality.rd')
save(df_65,file = 'all_cause_data/40-65_weekly_mortality.rd')
save(df_85,file = 'all_cause_data/65-85_weekly_mortality.rd')
save(df_max,file = 'all_cause_data/85+_weekly_mortality.rd')
